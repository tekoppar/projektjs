import { CMath, Prop, Item, CanvasDrawer, CanvasSprite, Rectangle, OperationType, Vector2D, CAnimation, AllAnimationsList } from '../../../internal.js'
...
class PlantData {
	 ...
	 * @param {{low:number, high:number}} gatherRange 
	...
	constructor(growthSpeed, regrowthSpeed, gatherRange = { low: 1, high: 4 }, plantIcon = new CanvasSprite(29, 10, 32, 32, 'fruitsveggies', true)) {
		/** @type {number} */ this.growthSpeed = growthSpeed;
		/** @type {number} */ this.growth = 0;
		/** @type {number} */ this.regrowthSpeed = regrowthSpeed;
		/** @type {{low: number, high: number}} */ this.gatherRange = gatherRange;
		/** @type {boolean} */this.hasFinishedGrowing = false;
		/** @type {boolean} */ this.hasBeenPicked = false;
		/** @type {CanvasSprite} */ this.plantIcon = plantIcon;
	}
...
class Plant extends Prop {
	 ...
	 */
	constructor(spriteSheetName, name, position, animations = undefined) {
		super(name, position, animations, spriteSheetName);

		/** @type {PlantData} */ this.plantData = AllPlantData[name].Clone();

		if (AllAnimationsList.plantAnimations[name] !== undefined)
			this.animations = AllAnimationsList.plantAnimations[name];

		if (this.animations !== undefined) {
			/** @type {CAnimation} */ this.currentAnimation = this.animations.seed.Clone();
	...
	PlantGathered(otherObject) {
		if (this.plantData.hasFinishedGrowing === true && this.plantData.hasBeenPicked === false && this.BoxCollision.GetCenterPosition().CheckInRange(otherObject.BoxCollision.GetCenterPosition(), 32.0) === true) {
			...
			this.CreateDrawOperation(frame, new Vector2D(this.GetPosition().x, this.GetPosition().y), true, this.canvas);
			this.currentAnimation.SetSpeed(this.plantData.growthSpeed);
